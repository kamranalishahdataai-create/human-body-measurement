...
flags.DEFINE_string('model_log_dir', 'logs', 'Where to save training models')
...
def prepare_dirs(config, prefix=['HMR']):
    ...
    if config.load_path:
        ...
        config.model_dir = osp.join(config.model_log_dir, save_name)

    for path in [config.model_log_dir, config.model_dir]:
